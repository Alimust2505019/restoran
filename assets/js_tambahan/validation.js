$(document).ready(function() {
	...
	$('#form-upload').submit(function(e){
		e.preventDefault();
		var fa = $(this);
		$('#form-upload').find(':submit').attr("disabled", "disabled");
		$('#form-upload').find(':submit').html("Menyimpan...");
		...
		$.ajax({
			...
			success: function(data){
				if (data.success == true) {
					...
					$('#form-upload').find(':submit').attr("disabled", false);
					$('#form-upload').find(':submit').html("Simpan");
					$.each(data.error_msg, function(key, value) {
						if (value) {
							console.log(value);
							$('#input-' + key).parents('.form-group').removeClass('has-success');
					...
					$('#form-upload input').parents('.form-group').addClass('has-error has-danger');		
					$('#form-upload').find(':submit').attr("disabled", false);
					$('#form-upload').find(':submit').html("Simpan");
					$('#false').html(data.false_msg);			            	
				}	
